    ...
    if models is None or model_df is None or feature_cols is None or horizons_engine is None:
        ...
        if intraday_df is not None and not intraday_df.empty:
            ...
            def get_scaled_ret_for(target_min: int) -> float | None:
                """
                엔진 horizon 중 가장 가까운 h_model의 future_ret_pred를 가져와서
                scale_mode에 따라 스케일링.
                - 선형(기본):   ret * (target/h_model)
                - 제곱근 완화: ret * sqrt(target/h_model)  (먼 미래일수록 덜 과격)
                - 스케일링 없음: ret 그대로 사용
                """
                ...
                if scale_mode == "선형(기본)":
                    scale = ratio
                elif scale_mode == "제곱근 완화":
                    scale = np.sqrt(ratio)
                else:  # "스케일링 없음"
                    scale = 1.0

                return float(base_ret * scale)
